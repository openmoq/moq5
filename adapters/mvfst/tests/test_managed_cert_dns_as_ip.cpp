/*
 * Regression test: cert with DNS SAN "127.0.0.1" (not IP SAN)
 * must be rejected when connecting to the IP literal 127.0.0.1.
 *
 * Per RFC 6125, IP literals must match iPAddress SANs, not dNSName.
 * A cert that only has DNS:127.0.0.1 should not satisfy an IP
...
/* -- Cert with DNS:127.0.0.1 but NO IP SAN -------------------------- */

struct test_cert { std::string cert_pem, key_pem; bool ok = false; };

static test_cert gen_cert_dns_as_ip()
{
    test_cert r;
    ...
    auto *san = X509V3_EXT_conf_nid(nullptr, &v3ctx,
                                     NID_subject_alt_name, "DNS:127.0.0.1");
    if (!san) return r;
...
/* ================================================================== */
/* Test: DNS:127.0.0.1 rejected for IP literal connection             */
/* ================================================================== */

static void test_dns_san_as_ip_rejected()
{
    auto cm = gen_cert_dns_as_ip();
    MVFST_CHECK(cm.ok);
    if (!cm.ok) return;

    char tmppath[] = "/tmp/moq_test_dnsip_XXXXXX";
