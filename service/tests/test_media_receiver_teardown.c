/*
 * Receiver destroy must CANCEL live subscriptions, not just free the facade.
 *
 * p2-medium__12: in attach mode moq_media_receiver_destroy() posted a teardown
 * task that called only moq_sub_destroy() -- facade-local cleanup that leaves the
 * session's catalog/media subscriptions ACTIVE. With the receiver hook detached,
 * nothing drains those subscriptions, so a continuing/malicious peer can keep
 * streaming objects into the still-alive endpoint.
 *
 * This drives the REAL receiver_hook (catalog SUBSCRIBE, catalog ingest,
 * auto-subscribe of the media track) over a simpair draft-16 client session to
 * an established catalog + video subscription, then runs the PRODUCTION teardown
 * path (receiver_take_teardown + receiver_destroy_sub_task, via the no-endpoint
 * test seam that supplies the session directly). It asserts the teardown emits an
 * UNSUBSCRIBE control message for BOTH the catalog and the media subscription.
 *
 * The teardown must cancel the live subscriptions (not merely destroy the
 * facade), so an UNSUBSCRIBE is emitted for each.
 */
...
/* -- Scripted-peer byte builders (mirrors test_media_receiver_scripted) -- */
...
/* Drain the client session's pending actions; learn the first two SUBSCRIBE
 * request ids (catalog, then video) and count UNSUBSCRIBE control messages. */
typedef struct {
...
int main(void)
{
    ...
    /* Drive to: catalog SUBSCRIBE+OK, catalog object -> CATALOG_READY, video
     * auto-SUBSCRIBE+OK. Both subscriptions then live in the session. */
    ...
    /* A few more cycles so the facade processes both SUBSCRIBE_OK events and the
     * catalog + video tracks settle to ACTIVE. */
    ...
    /* Preconditions: both subscriptions were issued + acknowledged, and the
     * action queue has been fully drained (so any UNSUBSCRIBE we count next is
     * from teardown, not a leftover). */
    ...
    /* Run the production teardown path with the live session. */
    now += 1000;
    moq_media_receiver_test_destroy_with_session(r, client, now);

    /* Teardown cancels BOTH live subscriptions (catalog + video) before
     * destroying the facade, so the session emits two UNSUBSCRIBE messages
     * (not zero, as a destroy-only teardown would). */
    drain_actions(client, &learned);
    fprintf(stderr, "RESULT: subscribes=%d unsubscribes=%d\n",
            learned.subscribe_seen, learned.unsubscribe_seen);
    MOQ_TEST_CHECK_EQ_INT(learned.unsubscribe_seen, 2);

    moq_simpair_destroy(sp);

    if (failures == 0)
        MOQ_TEST_PASS("media_receiver_teardown");
    return failures ? 1 : 0;
}
