/*
 * Managed client cert identity mismatch test.
 *
 * Verifies that a trusted cert with wrong IP SAN causes the
 * managed client to go fatal instead of silently connecting.
 *
...
static test_cert gen_cert_wrong_ip()
{
    ...
    auto *san = X509V3_EXT_conf_nid(nullptr, &v3ctx,
                                     NID_subject_alt_name, "IP:10.255.255.1");
    if (!san) return r;
...
/* -- Noop server (no session needed — TLS will fail before MoQ) ------ */
...
/* ================================================================== */
/* Test: identity mismatch — trusted cert, wrong IP                   */
/* ================================================================== */

static void test_cert_identity_mismatch()
{
    auto cm = gen_cert_wrong_ip();
    MVFST_CHECK(cm.ok);
    if (!cm.ok) return;

    char tmppath[] = "/tmp/moq_test_cert_mm_XXXXXX";
