/*
 * Managed mvfst client DNS hostname + TLS identity test.
 *
 * Verifies that a managed client can connect using a DNS hostname
 * (localhost) with a cert containing a matching DNS SAN.
 *
...
/* -- Cert with DNS SAN localhost + IP SAN 127.0.0.1 ------------------ */

struct test_cert { std::string cert_pem, key_pem; bool ok = false; };

static test_cert gen_cert_localhost()
{
    test_cert r;
...
/* ================================================================== */
/* Test: DNS loopback — connect via "localhost" with DNS SAN cert      */
/* ================================================================== */

static void test_dns_loopback()
{
    auto cm = gen_cert_localhost();
    MVFST_CHECK(cm.ok);
    if (!cm.ok) return;

    char tmppath[] = "/tmp/moq_test_dns_cert_XXXXXX";
    ...
    /* Connect via "localhost" — exercises DNS resolution + DNS SAN check. */
