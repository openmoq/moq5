...
#include <stddef.h>
#include <string.h>
static int pump(moq_wtquic_msquic_managed_t *m, moq_wtquic_msquic_managed_lane_t *lane, uint64_t now, void *ctx)
...
int main(void)
{
    ...
    assert(moq_wtquic_msquic_managed_create(&cfg, &fac) == MOQ_ERR_UNSUPPORTED);
    assert(!fac);
    cfg.setup_auth_tokens = NULL;
    assert(moq_wtquic_msquic_managed_create(&cfg, &fac) == MOQ_ERR_UNSUPPORTED);
    cfg.setup_auth_token_count = 0; cfg.setup_auth_tokens = &token;
    assert(moq_wtquic_msquic_managed_create(&cfg, &fac) == MOQ_ERR_UNSUPPORTED);
    union { moq_wtquic_msquic_managed_cfg_t cfg; unsigned char bytes[sizeof(cfg) + 16]; } canary;
    for (size_t n = sizeof(uint32_t); n <= sizeof(cfg); ++n) {
        memset(canary.bytes, 0xa5, sizeof(canary.bytes));
        moq_wtquic_msquic_managed_cfg_init_sized(&canary.cfg, n);
