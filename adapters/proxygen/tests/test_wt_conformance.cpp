/*
 * Proxygen WebTransport adapter conformance test.
 *
 * Runs the shared conformance scenarios through the real
 * moq::wt::Adapter + wt_endpoint_ops path.
 */
...
#define RUN_SCENARIO(name, required_caps) do { \
    moq_adapter_pair_t p = wt_conformance_create(); \
    if (!p.ops) { \
        std::fprintf(stderr, "FAIL: create for " #name "\n"); \
        fail_count++; \
    } else if ((required_caps) && \
               (p.capabilities & (required_caps)) != \
                   static_cast<uint32_t>(required_caps)) { \
        std::fprintf(stderr, "SKIP: " #name \
...
int main()
{
    ...
    std::printf("test_wt_conformance: %d run, %d skipped, %d failed\n",
